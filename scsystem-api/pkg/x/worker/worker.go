...
func StartServer(cnf *Worker) (*machinery.Server, error) {
...
func Launch(cnf *Worker, tag string, concurrency int) error {
	...
	errorHandler := func(err error) {
		log.ERROR.Println("error module:", err)
	}

	preTaskHandler := func(signature *tasks.Signature) {
		log.INFO.Println("start of task module for:", signature.Name)
	}

	postTaskHandler := func(signature *tasks.Signature) {
		log.INFO.Println("end of task module for:", signature.Name)
	}
...
func Execute(queue, taskName string, fn interface{}, args ...interface{}) error {
	if Conf == nil {
		return errors.New("worker config is required to launch")
	}
	worker := NewWorker(queue, Conf)
